...
def open_py_file():
    call(['python', 'EncodeGenerator4.py'])
open_py_file()
...
while True:
    ...
    if faceCurFrame:
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            if matches[matchIndex]:
                ...
                if counter == 0:
                    ...
                    cv2.imshow("Face ticket system", imgBackground)
                    cv2.waitKey(1)
                    counter = 1
                    modeType = 1
        ...
        if counter != 0:
            ...
            if counter == 1:
                # Get the Data
                studentInfo = db.reference(f'registrations/{id}').get()
                print(studentInfo)
                # Get the Image from the storage
                blob = bucket.get_blob(f'{folderImagePath}/{id}.png')
                array = np.frombuffer(blob.download_as_string(), np.uint8)
                imgStudent = cv2.imdecode(array, cv2.COLOR_BGRA2BGR)
                # Update data of attendance
                datetimeObject = datetime.strptime(studentInfo['last_use_time'], "%Y-%m-%d %H:%M:%S")
                secondsElapsed = (datetime.now() - datetimeObject).total_seconds()
                print(secondsElapsed)
                if secondsElapsed > 30:
                    ref = db.reference(f'registrations/{id}')
            ...
            if modeType != 3:
                ...
                if counter < 10:
                    ...
                    cv2.putText(imgBackground, str(studentInfo['wallet']), (1025, 625),
                                cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 100), 1)
                    cv2.putText(imgBackground, str(studentInfo['registration']), (1125, 625),
                                cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 100), 1)
